...
export async function GET() {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: `Weather API responded with status: ${res.status}` }, { status: res.status })
    }
    ...
    if (data.cod !== 200) {
      ...
      return NextResponse.json({ error: data.message || "Failed to fetch weather data" }, { status: 500 })
    }
